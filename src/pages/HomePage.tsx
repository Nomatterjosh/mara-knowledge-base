...
export default function HomePage({ studyState, onNavigate }: HomePageProps) {
  ...
  return (
    <div className="pb-24 px-4 pt-6 fade-in">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-1">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-0.5">Migration Agent Registration Assessment</p>
          </div>
      ...
      {/* Exam Info Banner */}
      <div className="card p-4 mb-5 border-l-4 border-primary-500 bg-gradient-to-r from-primary-50 to-white">
        <div className="flex items-start gap-3">
          <AlertCircle size={20} className="text-primary-600 flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-semibold text-primary-900">考试结构提醒</p>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <div className="card p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-gray-700">知识点进度</span>
          </div>
          <div className="mb-2">
          ...
          <p className="text-xs text-gray-400">必考项: {completedEssential}/{essentialTopics.length}</p>
        </div>

        <div className="card p-4">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 bg-accent-100 rounded-lg flex items-center justify-center">
              <HelpCircle size={16} className="text-accent-600" />
            </div>
            <span className="text-sm font-medium text-gray-700">题目练习</span>
          </div>
          <div className="text-2xl font-bold text-gray-900 mb-1">{attemptedQuizzes}</div>
          <p className="text-xs text-gray-500">已练习 / 共{totalQuestions}题</p>
      ...
      <div className="mb-5">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-gray-700">必考核心考点</h2>
          <button onClick={() => onNavigate('topics')} className="text-xs text-primary-600 flex items-center gap-0.5">
          ...
          {essentialTopics.slice(0, 5).map(topic => {
            ...
            return (
              <button
                ...
                className="card w-full text-left p-3 flex items-center gap-3 hover:bg-gray-50"
              >
      ...
      {recentTopics.length > 0 && (
        <div className="mb-5">
          <h2 className="text-sm font-semibold text-gray-700 mb-3">最近学习</h2>
          <div className="space-y-2">
            {recentTopics.map(topic => (
              <div key={topic.id} className="card p-3 flex items-center gap-3">
      ...
      <div className="card p-4 bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200">
        <h3 className="text-sm font-semibold text-amber-900 mb-2">💡 Capstone 备考要点</h3>
